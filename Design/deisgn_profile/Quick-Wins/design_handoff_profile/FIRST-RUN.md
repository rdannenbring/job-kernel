...
4. **Quick wins checklist** — a `PSection` titled "Quick wins" with "0 of N done", listing
   ...
   **Each incomplete row is a navigation link to its section** — make this legible *at rest*,
   not just on hover (a finger cursor alone is insufficient because it only appears after
   hovering). Implementation:
   - **Done rows:** filled green check, muted label, muted `+N`, **no chevron**, default
     cursor, **not interactive** — reads as terminal.
   - **To-do rows:** hollow primary circle, full-strength label, primary `+N` impact, and a
     **trailing chevron (›)** in a muted tone — all visible at rest, so the row reads as
     tappable. On hover: row background fills, label turns `--primary`, the word "Complete"
     slides in before the chevron, chevron nudges right + turns primary, finger cursor.
   - Clicking a to-do row jumps to + opens that section in edit mode (same scroll-to-section
     behavior as ⌘K — compute offset, `scrollTo`, never `scrollIntoView`).
